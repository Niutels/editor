...
function summarizeBusEvent(event: unknown): unknown {
  ...
  const pos = rec.position as
    | readonly [number, number, number]
    | { x?: number; y?: number; z?: number }
    | undefined
  if (Array.isArray(pos) && pos.length >= 3) {
    out.position = pos.slice(0, 3).map((value) => Math.round(value * 100) / 100)
  } else if (pos && !Array.isArray(pos)) {
    const objectPosition = pos as { x?: number; y?: number; z?: number }
    if (typeof objectPosition.x === 'number') {
      out.position = [
        Math.round((objectPosition.x ?? 0) * 100) / 100,
        Math.round((objectPosition.y ?? 0) * 100) / 100,
        Math.round((objectPosition.z ?? 0) * 100) / 100,
      ]
    }
  }
  const native = rec.nativeEvent as
    | { clientX?: number; clientY?: number; button?: number; detail?: number }
    | undefined
  if (native && typeof native.clientX === 'number') {
    out.client = [native.clientX, native.clientY]
    out.button = native.button
    out.detail = native.detail
  }
