...
export const LANDRUSH_ISLAND_LOADING_SHELL_BOOTSTRAP_SOURCE = `(()=>{var w=window,p=performance,l=location,d=document,c=d.currentScript,s=c&&c.closest('[data-landrush-island-loading-shell]'),r=l.pathname+l.search,e=w.navigation&&w.navigation.currentEntry,k=e&&typeof e.key==='string'?'entry:'+e.key:'route:'+r,g=w.${LANDRUSH_ISLAND_LOADING_BOOT_RUN_GLOBAL},promote=g&&g.routeKey===r&&g.navigationKey==='route:'+r&&k.indexOf('entry:')===0,reuse=g&&g.version===${LANDRUSH_ISLAND_LOADING_BOOT_CONTRACT_VERSION}&&(g.navigationKey===k||promote)&&g.routeKey===r&&g.owner!=='complete';if(!reuse){var q=(Number(w.${LANDRUSH_ISLAND_LOADING_BOOT_SEQUENCE_GLOBAL})||0)+1;w.${LANDRUSH_ISLAND_LOADING_BOOT_SEQUENCE_GLOBAL}=q;g={navigationKey:k,owner:'shell',routeKey:r,runId:'landrush-'+String(Math.round(Number(p.timeOrigin)||Date.now()))+'-'+String(q),startedAtMs:p.now(),version:${LANDRUSH_ISLAND_LOADING_BOOT_CONTRACT_VERSION}};}else if(promote)g.navigationKey=k;w.${LANDRUSH_ISLAND_LOADING_BOOT_RUN_GLOBAL}=g;if(s){s.setAttribute('${LANDRUSH_ISLAND_LOADING_SHELL_RUN_ATTRIBUTE}',g.runId);s.style.setProperty('${LANDRUSH_ISLAND_LOADING_SHELL_DELAY_PROPERTY}',String(-Math.max(0,p.now()-g.startedAtMs))+'ms')}})();`
