...
export function useLandrushIslandLoadingTimeline({
  ...
  useEffect(() => {
    ...
    const percent =
      overlay?.querySelector<HTMLElement>('[data-landrush-island-loading-shell-percent]') ?? null
    const percentReel =
...
export function createLandrushIslandLoadingPercentKeyframes(
  ...
  for (let percent = firstPercent + 1; percent <= lastPercent; percent += 1) {
    const completeKeyframe =
      percent === 100 ? segment.keyframes.find((keyframe) => keyframe.progress === 1) : undefined
    if (completeKeyframe) {
      frames.push({
        easing: 'steps(1, end)',
        offset: completeKeyframe.offset,
