...
describe('Landrush island material presentation ownership', () => {
  ...
  test('owns every array slot and restores the exact original material array', () => {
    ...
    expect(combinedMaterials[0]?.transparent).toBe(true)
  ...
  test('puts every fractional reveal on a sorted blending pipeline', () => {
    const owner = new LandrushIslandMaterialPresentationOwner()
    const transparentSource = new MeshBasicMaterial({ depthWrite: false, transparent: true })
    const transmissionSource = new MeshPhysicalMaterial({ transmission: 0.8 })
  ...
  test('preserves authored cutout modes without adding a presentation alpha test', () => {
    ...
    expect(hashedReveal.transparent).toBe(true)
    expect(hashedReveal.depthWrite).toBe(false)
    ...
    expect(coverageReveal.transparent).toBe(true)
    expect(coverageReveal.depthWrite).toBe(false)
    ...
    expect(alphaTestReveal.transparent).toBe(true)
    expect(alphaTestReveal.depthWrite).toBe(false)
