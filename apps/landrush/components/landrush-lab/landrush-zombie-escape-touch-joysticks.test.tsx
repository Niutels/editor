...
describe('Landrush Zombie Escape touch joysticks', () => {
  ...
  test('renders both visible sticks and an accessible jump action on a touch device', () => {
    ...
    expect(markup.match(/touch-none select-none/g)).toHaveLength(4)
    expect(markup.match(/\[-webkit-tap-highlight-color:transparent\]/g)).toHaveLength(4)
    expect(markup.match(/\[-webkit-touch-callout:none\]/g)).toHaveLength(4)
    expect(markup.match(/\[-webkit-user-select:none\]/g)).toHaveLength(4)
