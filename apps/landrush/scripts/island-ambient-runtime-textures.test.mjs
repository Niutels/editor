...
const playerRobotAssetName = 'proto_pascal_robot-bc1-47d84fc6.glb'
const playerRobotAssetPublicPath = `/navigation/${playerRobotAssetName}`
const debugRobotAssetName = 'proto_pascal_robot-ktx2-1112f038.glb'
const debugRobotAssetPublicPath = `/navigation/${debugRobotAssetName}`
...
test('runtime textured ambient GLBs use the KTX2-aware loader from their first request', async () => {
  ...
  assert.match(robotAssetSource, new RegExp(escapeRegExp(playerRobotAssetPublicPath), 'u'))
  assert.match(robotAssetSource, /\/navigation\/proto_pascal_robot\.glb/u)
  assert.match(orbotDebugSource, /useGLTFKTX2\(ORBOT_ASSET_PATH\)/u)
  assert.match(orbotDebugSource, new RegExp(escapeRegExp(debugRobotAssetPublicPath), 'u'))
  assert.doesNotMatch(orbotDebugSource, /useGLTF\(ORBOT_ASSET_PATH\)/u)
})

test('player robot uses a fingerprinted native BC1 mip chain while retaining its legacy URL', async () => {
  ...
  const [optimizedBuffer, legacyBuffer] = await Promise.all([
    readFile(resolve(navigationRoot, playerRobotAssetName)),
  ...
  assert.equal(
    ...
    playerRobotAssetName.match(/-([a-f0-9]{8})\.glb$/u)?.[1],
  )
  ...
  assert.equal(ktx2.readUInt32LE(12), 132)
  assert.equal(ktx2.readUInt32LE(44), 0)
  let mipByteLength = 0
  for (let level = 0; level < 11; level += 1) {
    mipByteLength += Number(ktx2.readBigUInt64LE(80 + level * 24 + 8))
  }
  assert.equal(mipByteLength, 699_064)
})
