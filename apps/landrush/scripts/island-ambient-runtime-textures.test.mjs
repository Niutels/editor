...
test('runtime textured ambient GLBs use the KTX2-aware loader from their first request', async () => {
  ...
  assert.match(robotSource, /useGLTFKTX2\(LANDRUSH_ROBOT_ASSET_PATH\)/u)
  assert.match(robotSource, new RegExp(escapeRegExp(robotAssetPublicPath), 'u'))
  assert.doesNotMatch(robotSource, /useGLTF\(LANDRUSH_ROBOT_ASSET_PATH\)/u)
  assert.doesNotMatch(robotSource, /useGLTF\.preload\(LANDRUSH_ROBOT_ASSET_PATH\)/u)
