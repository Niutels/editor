...
export function createPascalWaterFieldTexture({
  parameters,
  perimeter,
  planeSize,
...
export function createPascalWaveDepthFieldTexture(
  ...
  if (!source) {
    return createWaterFieldTextureFromData(bytes, width, height)
  }
...
function createWaterFieldTextureFromData(bytes: Uint8Array, width: number, height: number) {
