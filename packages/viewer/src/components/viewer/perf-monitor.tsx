...
export const INITIAL_PERF_STATS: PerfStats = {
  fps: 0,
  frameMs: 0,
  gpuMs: 0,
  gpuMaxMs: 0,
...
export const PerfOverlay = ({ stats }: PerfOverlayProps) => {
  const panelText = [
    `FPS    ${stats.fps}`,
    `GPU    ${
      stats.gpuMs > 0 ? `${stats.gpuMs.toFixed(1)}ms (max ${stats.gpuMaxMs.toFixed(1)})` : '-'
    }`,
